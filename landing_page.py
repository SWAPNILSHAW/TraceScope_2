...
def generate_forensic_pdf(
    ...
    with PdfPages(buf) as pdf:
        ...
        ax_t.set_xlim(0, 1)
        ax_t.set_ylim(0, 1)
        ax_t.text(0.0, 1.04, "3-TIER MODEL ATTRIBUTION BREAKDOWN", fontsize=9, fontweight="bold", color="#1e293b", transform=ax_t.transAxes)
        
        def _get_hw_label(c_raw):
            b, m = format_scanner_display(c_raw)
            full = m if m.startswith(b) else f"{b} {m}"
            if " (" in full:
                parts = full.split(" (", 1)
                return f"{parts[0]}\n({parts[1]}"
            if len(full) > 22 and " " in full:
                idx = full[:22].rfind(" ")
                if idx != -1:
                    return f"{full[:idx]}\n{full[idx+1:]}"
            return full

        hw_desc_wrapped = hw_desc
        if " (" in hw_desc_wrapped:
            parts = hw_desc_wrapped.split(" (", 1)
            hw_desc_wrapped = f"{parts[0]}\n({parts[1]}"
        elif len(hw_desc_wrapped) > 22 and " " in hw_desc_wrapped:
            idx = hw_desc_wrapped[:22].rfind(" ")
            if idx != -1:
                hw_desc_wrapped = f"{hw_desc_wrapped[:idx]}\n{hw_desc_wrapped[idx+1:]}"
        ...
        t_data = [
            ...
            ["Tier 3: Flagship Hybrid", "Dual-Branch Fusion\n(Residual + 44 Descriptors)", _get_hw_label(results_dict.get("hybrid", {}).get("class", primary_result.get("model", ""))), f"{results_dict.get('hybrid', {}).get('confidence', 82.4)}%", "Open-Set Validated"],
            ["Consensus Engine", "Multi-Model Bayesian Calibration & Voting", hw_desc_wrapped, f"{primary_result['confidence']}%", "Decisive Attributed"]
        ]
        tbl = ax_t.table(
            cellText=t_data, 
            colLabels=["Model Tier", "Architecture / Methodology", "Predicted Hardware", "Confidence", "Status"], 
            colWidths=[0.16, 0.31, 0.28, 0.11, 0.14],
        ...
        ax_f.set_xlim(0, 1)
        ax_f.set_ylim(0, 1)
        ax_f.text(0.0, 0.3, "Official Forensic Dossier • Generated by TraceScope AI 2.0 • Page 1 of 2", fontsize=7.5, color="#94a3b8", transform=ax_f.transAxes)
        ax_f.text(1.0, 0.3, "SWAPNIL SHAW", fontsize=7.5, fontweight="bold", color="#0f172a", ha="right", transform=ax_f.transAxes)
        ...
        ax2_f.text(1.0, 0.3, "SWAPNIL SHAW", fontsize=7.5, fontweight="bold", color="#0f172a", ha="right", transform=ax2_f.transAxes)
